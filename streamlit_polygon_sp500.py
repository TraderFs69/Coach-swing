...
min_conditions = st.slider("🔢 Nombre minimal de conditions à remplir", min_value=3, max_value=5, value=5)
...
def calculate_indicators(df):
    ...
    obv_cond = obv > obv_sma20
        ...
        if count.iloc[-1] >= 3:
            print(f"🔎 {ticker} : {count.iloc[-1]}/5 conditions remplies")
        count = ut_buy.astype(int) + macd_cond.astype(int) + stoch_cond.astype(int) + adx_cond.astype(int) + obv_cond.astype(int)
        if count.iloc[-1] >= 3:
            print(f"🔎 Ticker avec signal partiel : {count.iloc[-1]}/5 conditions")
...
st.write(f"✅ {len(tickers)} tickers analysés.")
        import matplotlib.pyplot as plt
        counts = [ut_buy.iloc[-1], macd_cond.iloc[-1], stoch_cond.iloc[-1], adx_cond.iloc[-1], obv_cond.iloc[-1]]
        labels = ["UT Buy", "MACD", "Stoch", "ADX", "OBV"]
        fig, ax = plt.subplots()
        ax.bar(labels, counts, color="skyblue")
        ax.set_title("✅ Conditions remplies pour le dernier jour")
        st.pyplot(fig)
